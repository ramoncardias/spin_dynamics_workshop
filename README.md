...
3. a frustrated Kagomé antiferromagnet with chiral 120° order.
...
[
E=-sum_{i<j}J_{ij}mathbf S_icdotmathbf S_j
-sum_{i<j}mathbf D_{ij}cdot(mathbf S_i	imesmathbf S_j)
-sum_imathbf Bcdotmathbf S_i,
]

then integrates the Gilbert form of the LLG equation with a Heun predictor–corrector
scheme. A finite-difference test verifies that the implemented effective field equals
(-partial E/partialmathbf S_i).
...
- `scripts/validate_notebook.py`: structural and Python-syntax checks
